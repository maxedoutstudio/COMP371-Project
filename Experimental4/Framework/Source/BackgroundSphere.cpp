...
BackgroundSphere::BackgroundSphere(int shipTextureID,glm::vec3 size) : Model()
{
...
BackgroundSphere::~BackgroundSphere()
{
	...
	backgroundScene->~sceneLoader();
}

void BackgroundSphere::Update(float dt)
{
...
bool BackgroundSphere::ParseLine(const std::vector<ci_string> &token)
{
	if (token.empty())
	{
		return true;
