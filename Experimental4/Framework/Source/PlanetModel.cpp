...
PlanetModel::PlanetModel(int shipTextureID,bool rotSelf,bool rotOrbit,glm::vec4 surfaceProperties,glm::vec3 size) : Model()
{
	selfRotation = rotSelf;
	orbitRotation = rotOrbit;
	ka = surfaceProperties[0];
	kd = surfaceProperties[1];
	ks = surfaceProperties[2];
	n = surfaceProperties[3];

	mTextureID = shipTextureID;
	planetScene = World::GetInstance()->earthScene;
